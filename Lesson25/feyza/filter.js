...
const filteredBookPrices = booksExercise9.filter(book => book.price <= 10);
console.log(filteredBookPrices);
...
const filteredAges = usersExercise4.filter(people => people.age <= 25);
...
const filteredSentences = sentencesExercise5.filter(word => word.split(' ').length >= 10);
...
const filteredQuantity = productsExercise6.filter(product => product.quantity <= 0);
...
const divisibleNumbers = numbersExercise7.filter(number => number % 3 == 0);
...
const filteredCharacters = wordsExercise8.filter(word => word.length >= 5);
...
const filteredGradeLower = studentsExercise9.filter(student => !passingGrades.includes(student.grade));

console.log(filteredGradeLower);
...
const filteredNames = usersExercise10.filter(user => user.name.startsWith("J"));

console.log(filteredNames);

//Anna 4 dosyada bulunan bütün exerciseleri kendim yapamaya çalıştım ve %85 başarılı oldum. Bu da benim için motive edici. :)