...
console.log(wordsExercise4.map(wordsLength => wordsLength.length));
...
console.log(studentNamesAndGrades); // Output: ['Alice A', 'Bob B', 'Charlie C']
...
//ilk olarak kendim methodu sadece split() olarak yazdım. Sadece cümleleri array içine aldı. 
//Daha sonra chatGPT den split(' ') şeklinde yazmam gerektiğini öğrendim.
...
const userEmailAddress = usersExercise8.map(person => `${person.email}`);
...
const bookTitleAndAuthors = booksExercise9.map(bookinfo => `${bookinfo.title} by ${bookinfo.author}`);

console.log(bookTitleAndAuthors);
