...
const filteredBooks = booksExercise9.filter(book => book.price >= 10);
...
const filteredUsers = usersExercise4.filter(user => user.age >= 25);
...
const filteredStocks = productsExercise6.filter(product => product.quantity !== 0);
...
const filteredNumbersDivisibleByThree = numbersExercise7.filter(number => number % 3 !== 0);
console.log(filteredNumbersDivisibleByThree);
...
const filteredCharacters = wordsExercise8.filter(string => string.length <= 5);
...
const filteredStudentsGrades = studentsExercise9.filter(student => grades.indexOf(student.grade) <= grades.indexOf('C'));
console.log(filteredStudentsGrades);
...
const userObjectsNames = usersExercise10.filter(object => !object.name.startsWith("J"));
console.log(userObjectsNames);