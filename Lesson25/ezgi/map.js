...
console.log(
 studentsExercise5.map(item =>`${item.name} ${item.grade}`)
);
...
console.log(
	numbersExercise6.map(number=> number.toString())
);
...
console.log(usersExercise8.map(object => object.email));
...
console.log( pricesExercise10.map(number => `$${number}`)
);
