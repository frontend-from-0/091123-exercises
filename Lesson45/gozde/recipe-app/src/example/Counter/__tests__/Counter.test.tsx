...
import { Counter, counterReducer, CounterActionType } from "..";
...
describe("counterReducer", () => {
  it("should increment the count by 1 when the action type is INCREMENT", () => {
    ...
    const action = { type: CounterActionType.INCREMENT };
  ...
  it("should decrement the count by 1 when the action type is DECREMENT", () => {
    const initialState = { count: 0 };
    const action = { type: CounterActionType.DECREMENT };
    const newState = counterReducer(initialState, action);
    expect(newState.count).toBe(-1);
  });
  ...
  it("should not increment or decrement the count when the action type does not match DECREMENT or INCREMENT", () => {
    const initialState = { count: 0 };
    const action = { type: "UNKNOWN" as unknown as CounterActionType }; // bilinmeyen eylem türü için ts deki hata bu şekilde çözüldü
    expect(() => counterReducer(initialState, action)).toThrow();
  });
});
